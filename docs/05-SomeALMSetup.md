...
> [!WARNING]
> GitHub Copilot suggestions are not always valid, so be careful and validate the code generated by it. We will use the time allocated to the next chapter and the fact that you will have some code to test to finalize the configuration of the GitHub workflow and make it work, don't worry.
